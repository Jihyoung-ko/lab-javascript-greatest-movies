...
function getAllDirectors(arrOfMovies){
  ...
  arrOfDirectors = arrOfMovies.map(function(movie){
...
function howManyMovies(arrOfMovies){
  ...
  arrOfDramaMovie = arrOfMovies.filter(function(movie){
    return movie.genre.includes("Drama") && movie.director === "Steven Spielberg"
  })
  return arrOfDramaMovie.length
}
...
function dramaMoviesRate(arrOfMovies) {
  ...
  return ratesAverage(arrOfDramaMovie);
}
...
function orderByYear(arrOfMovies){
  ...
  arrOfSortMovie = arrOfMovies.sort(function(a, b){
...
function orderAlphabetically(arrOfMovies) {
  ...
  let tempArrOfMovies = [...arrOfMovies];

  arrOfSortMovie = tempArrOfMovies.sort((a, b) => (a.title > b.title) ? 1 : -1)

  arrOfSortMovie = arrOfSortMovie.slice(0, 20);

  let arrOfSortMovieTitles = arrOfSortMovie.map(function(movie){
    return movie.title;
  })

  return arrOfSortMovieTitles;
}
...
function turnHoursToMinutes(arrOfMovies){
  ...
  let arrOfSortMovie =  [];
  let arrOfSortMovieTemp = [...arrOfMovies];

  arrOfSortMovie = arrOfSortMovieTemp.map(function(movie){
    let hours = movie.duration.includes('h');
    let minutes = movie.duration.includes('min');
    let timeStampMins = 0;
    if(!minutes){
      hour = movie.duration.split('h')[0];
      timeStampMins = parseInt(hour) * 60;
    }else if(!hours){
      minutes = movie.duration.split('min')[0];
      timeStampMins = parseInt(minutes);
    }else{
      hour = movie.duration.split('h')[0];
      minutes = movie.duration.split('h')[1].split('min')[0];
      timeStampMins = parseInt(hour) * 60 + parseInt(minutes);
    }
  

    return {...movie, duration:timeStampMins}
  });
